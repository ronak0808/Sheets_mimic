...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.removeDuplicates = function () {
        ...
        document.querySelectorAll("td").forEach(cell => {
            let value = cell.innerText.trim().toLowerCase();  // ✅ Normalize text
            if (value && seen.has(value)) {
                cell.innerText = "";  // ✅ Clear duplicate content
            } else {
                seen.add(value);
            }
        });
    ...
    window.formatText=function(style) {
        if (selectedCell) {
            if (style === 'bold') selectedCell.style.fontWeight = selectedCell.style.fontWeight === 'bold' ? 'normal' : 'bold';
            if (style === 'italic') selectedCell.style.fontStyle = selectedCell.style.fontStyle === 'italic' ? 'normal' : 'italic';
        }
    };
    ...
    window.saveSpreadsheet = function () {
        ...
        if (!sheetId) {
          ...
          let sheetNameElement = document.getElementById("sheet-name");
          let sheetName = sheetNameElement ? sheetNameElement.innerText.trim() : "Untitled Sheet";
